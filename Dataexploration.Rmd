...
```{r}
df_coef_final <- cbind(summary(mod.fit)$coefficients[,1:2] %>% round(2), `p-value` = formatC(summary(mod.fit)$coefficients[,4], format = "e", digits = 2))

rownames(df_coef_final) <- c("Intercept", "Fewer Trips to Store", "Fewer Trips on Transit", "Remote Learning", "Canceled Vacation")
#k1 <- kable(df_coef_final)
...
png(file="upset.png") # or other device
