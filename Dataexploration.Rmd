...
```
Use cutoff of 60 because of the children variable. We want to assess family unit 
...
```{r}
#ENROLL1
#df_school_age <- filter(df, ENROLL1 ==1 | ENROLL2  == 1)

ggplot(df_school_age, aes(x = age)) +
  geom_histogram()

df_school_age_agg <- group_by(df, issue, telework, fewer_trips, age, fewer_trips_transit, children_learn_from_home, behavior_change_due_risk, canceled_trip_due_to_covid, school_children) %>%
  summarize(n = n())

mod.fit_school_age_agg <- glm(issue/n ~ telework + fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_school_age_agg, weights = n)

pchisq(mod.fit_school_age_agg$deviance, mod.fit_school_age_agg$df.residual, lower.tail = F)

plot(mod.fit_school_age_agg)
```

```{r}
df_young_agg_all_var <- group_by(df_young, issue, telework, fewer_trips, age, fewer_trips_transit, children_learn_from_home, behavior_change_due_risk, canceled_trip_due_to_covid) %>%
  summarize(n = n())
...
mod.fit_young_cont_agg_no_age_no_tele <- glm(issue/n ~ fewer_trips + fewer_trips_transit + children_learn_from_home + behavior_change_due_risk + canceled_trip_due_to_covid, family = binomial, data=df_young_agg_all_var_no_age_no_tele, weights = n)
...
```{r}
df_young_agg_all_var_no_age_no_tele_no_behave <- group_by(df_young, issue, fewer_trips, fewer_trips_transit, children_learn_from_home, canceled_trip_due_to_covid) %>%
  summarize(n = n())

mod.fit_young_cont_agg_no_age_no_tele_no_behave <- glm(issue/n ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_young_agg_all_var_no_age_no_tele_no_behave, weights = n)
...
summary(mod.fit_young_cont_agg_no_age_no_tele_no_behave)

print(mod.fit_young_cont_agg_no_age_no_tele_no_behave$aic) %>% round(2)

ggplot(df_young_agg_all_var_no_age_no_tele_no_behave, aes(x = fewer_trips, y = n)) +
  geom_bar(position="dodge", stat="identity")
```


```{r}
mod.fit_school_age <- glm(issue/n ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_young_agg_all_var_no_age_no_tele_no_behave, weights = n)
...
```{r}
mod.fit_young_cont_agg_no_age_no_tele_no_behave_rv_outlier <- glm(issue/n ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_young_agg_all_var_no_age_no_tele_no_behave[2:nrow(df_young_agg_all_var_no_age_no_tele_no_behave),], weights = n)

pchisq(mod.fit_young_cont_agg_no_age_no_tele_no_behave_rv_outlier$deviance, mod.fit_young_cont_agg_no_age_no_tele_no_behave_rv_outlier$df.residual, lower.tail = F)
plot(mod.fit_young_cont_agg_no_age_no_tele_no_behave_rv_outlier)

influencePlot(mod.fit_young_cont_agg_no_age_no_tele_no_behave)
summary(mod.fit_young_cont_agg_no_age_no_tele_no_behave_rv_outlier)
```
