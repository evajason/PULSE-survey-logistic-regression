...
ggplot(df_groups, aes(x=age_group, y = percent, fill = has_issue)) +
  ...
  theme_minimal() +
  scale_fill_discrete(name = "Has Mental Health Issue") +
...
df_agg_all_var <- group_bydf_young, telework, fewer_trips, age, fewer_trips_transit, children_learn_from_home, behavior_change_due_risk, canceled_trip_due_to_covid) %>%
  ...
  mutate(proportion = total_w_issue/n)
...
mod.fit_young_cont_agg_no_age_no_tele <- glm(proportion ~ fewer_trips + fewer_trips_transit + children_learn_from_home + behavior_change_due_risk + canceled_trip_due_to_covid, family = binomial, data=df_young_agg_all_var_no_age_no_tele, weights = n)
...
write.table(df_agg,"aggregated_data.csv", sep=",", row.names=F)

df_agg <- read.csv("aggregated_data.csv")

mod.fit <- glm(proportion ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_agg, weights = n)

#mod.fit_full <- glm(issue ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_young)

pchisq(mod.fit$null.deviance, mod.fit$df.null, lower.tail = F)
...
generalhoslem::logitgof(df_young$issue, mod.fit_full$fitted.values)

df_agg2 <- df_agg[2:nrow(df_agg),]

mod.fit2 <- glm(proportion ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_agg2, weights = n)

pchisq(mod.fit2$deviance, mod.fit2$df.residual, lower.tail = F)

#mod <- logistf(issue ~ fewer_trips + fewer_trips_transit + children_learn_from_home + canceled_trip_due_to_covid, family = binomial, data=df_young)
```
...
prop_coefs <- expit(coef(mod.fit)["(Intercept)"] + mod.fit$coefficients) %>% data.frame() 

odds_coefs <- exp(mod.fit$coefficients) %>% data.frame() 
```
...
prob_change <- round(expit(summary(mod.fit)$coefficients[,1]) %>% as.numeric(),2)
...
kable(df_coef_final, "latex", booktabs = T) %>%
...
```{r}
df_agg_plt <- arrange(df_agg, desc(fitted))
  
df_agg_plt$Rank <- seq(1,nrow(df_agg_plt),1)
df_agg_plt$total <- df_agg_plt$fewer_trips + df_agg_plt$fewer_trips_transit + df_agg_plt$children_learn_from_home + df_agg_plt$canceled_trip_due_to_covid
#df_agg_plt$prob <- exp(df_agg_plt$fitted)
ggplot(df_agg_plt, aes(x = factor(Rank), y = fitted,color = factor(total))) +
  #geom_col(aes(fill = factor(total))) +
  geom_point(aes(x = factor(Rank), y = proportion), color ="black", size=3) +
  geom_point(size = 3) +
  theme_minimal() +
  xlab("Rank") +
...
ggplot(df_agg_plt, aes(x = factor(Rank), y = fitted, fill = factor(total))) +
  #geom_col(aes(fill = factor(total))) +
  #geom_point(aes(x = factor(Rank), y = proportion), color ="black", size=3) +
  geom_col() +
  theme_minimal() +
  scale_x_discrete(labels = NULL) + labs(x = NULL, y=NULL) +
  #xlab("Rank") +
  ggtitle("Probability of a Mental Health Issue") +
  #ylab("Probability") +
  ylim(c(0,1)) +
...
df_agg_table <- select(df_agg_plt,fewer_trips, fewer_trips_transit, children_learn_from_home, canceled_trip_due_to_covid, fitted)
df_agg_table$fitted_val <- round(df_agg_table$fitted, 3)
