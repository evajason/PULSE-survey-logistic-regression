...
```{r}
df_mod <- select(df, issue,TW_START, FEWRTRIPS, FEWRTRANS, PLNDTRIPS, CNCLDTRPS, TEACH1, TEACH2, TEACH3, TEACH4, TEACH5, behavior_change_due_risk, canceled_trip_due_to_covid) %>%
  mutate(across(everything(), as.character)) %>%
  mutate(issue = as.numeric(issue))
mod.fit <- glm(issue ~ TW_START + FEWRTRIPS + FEWRTRANS + PLNDTRIPS + CNCLDTRPS + TEACH1 + TEACH2 + TEACH3 + TEACH4 + TEACH5 + behavior_change_due_risk + canceled_trip_due_to_covid, family = binomial, data=df_mod)
```
